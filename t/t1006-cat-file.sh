...
test_expect_success 'cat-file --batch-all-objects --no-dedup shows duplicates' '
	git -C all-two cat-file --batch-all-objects \
				--batch-check="%(objectname)" \
				--no-dedup >actual.unsorted &&
	sort -u actual.unsorted >actual &&
	test_cmp expect actual
'
...
test_expect_success 'cat-file --unordered --no-dedup works' '
	git -C all-two cat-file --batch-all-objects --unordered \
				--batch-check="%(objectname)" \
				--no-dedup >actual.unsorted &&
	sort -u <actual.unsorted >actual &&
