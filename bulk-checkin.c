				     ...
				     const char *path, unsigned flags)
{
	...
	if (*already_hashed_to < size) {
		...
		if (hsize) {
			...
			if (compat)
				compat->update_fn(compat_ctx, buf, hsize);
		}
...
static void finalize_checkpoint(struct bulk_checkin_packfile *state,
				git_hash_ctx *ctx,
				git_hash_ctx *compat_ctx,
				struct hashfile_checkpoint *checkpoint,
				struct pack_idx_entry *idx,
				struct object_id *result_oid,
				const struct git_hash_algo *compat)
{
					       ...
					       const char *path, unsigned flags)
{
	...
	while (1) {
		...
		if (!stream_obj_to_pack_incore(state, ctx, compat_ctx,
					       &already_hashed_to,
					       buf, size, type,
					       path, flags))
			break;
	...
	finalize_checkpoint(state, ctx, compat_ctx, &checkpoint, idx,
			    result_oid, compat);
				...
				const char *path, unsigned flags)
{
	...
	finalize_checkpoint(state, &ctx, &compat_ctx, &checkpoint, idx,
			    result_oid, the_repository->compat_hash_algo);
	return 0;
}
